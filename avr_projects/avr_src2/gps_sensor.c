...
/* This is extern for testing only */
extern void gps_parse_string(uint8_t, uint8_t, uint8_t, uint8_t);

//volatile unsigned char gpsConvertState = NO_STATE;
volatile unsigned char gpsCaptureState = FALSE;

char gps_data(int value)
{
	gps_send_msg(GPS_QRY_GGA_MSG);
	gpsCaptureState = TRUE;
	while (gpsCaptureState);
	gps_parse_gga();
	return gpsParsed[value];
}
...
ISR (USART_RX_vect)
{
	...
	//	if (gpsConvertState == WAIT_GGA) gpsConvertState = PARSE_GGA;
	//	if (gpsConvertState == WAIT_RMC) gpsConvertState = PARSE_RMC;
	}
