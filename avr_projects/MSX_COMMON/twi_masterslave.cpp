...
ISR (TWI_vect)
{
	if (twiState == TWI_MASTER) {
		switch (TWSR & 0xF8){
			...
			case SLAR_DAT_NACK:
				...
				TWCR = TWIMASTERSTOP;
			break;
			/* This case represents a bus error due to an illegal START or STOP condition. The 
			 TWI returns to a well-define unaddressed Slave mode and releases SDA and SCL. */
			case ILLEGAL_ST:
				TWCR = (1<<TWSTO) | (1<<TWINT);
				//TWCR = TWISLAVEDISABLE;
				//TWCR = TWISLAVEPASSIVE;
			break;
			/* Arbitration has been lost. Release SDA and SCL and resend START */
			case SLAW_ARB_LOST:
	...
	//			}
				...
				break;
				/* If you reach this point, arbitration has been lost */
				case SLAR_ARB_LOST:
					TWCR = (1<<TWINT);
				break;
				/* This case represents a bus error due to an illegal START or STOP condition. The 
				 TWI is shut down to release the lines and reinitialized. It not entirely clear
				 why it needs to send an ACK but this is working*/
				case ILLEGAL_ST:
					TWCR = TWISLAVEDISABLE;
					TWCR = TWISLAVEACTIVEACK;
				break;
