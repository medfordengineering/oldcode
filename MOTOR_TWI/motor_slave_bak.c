...
static volatile unsigned int pulseCounter[4];

static volatile unsigned int rcChannel;
static volatile unsigned int pulseChannel;
...
static volatile unsigned char captureState;
...
ISR (TIMER1_CAPT_vect)
{
	...
	if (captureState == 1) {
		...
		/* Captured on low transition */
			...
			pulseCounter[pulseChannel]++;
			if (pulseChannel++ >= 4) pulseChannel = 0;
			set_rcChannel(pulseChannel);
	}
	sei();
}
...
int main(void)
{
	unsigned int current, pulse, counter;
	signed int speed;
	...
	while (TRUE) {
		if (twiDataInBuffer) {
			switch (twiData[0]) {
				...
				break;
				case CMD_PULSE_COUNTER:
					captureState = 0;
					hcmd_initialize_rc(ENABLE);
					counter = pulseCounter[twiData[1]];
					twiData[0] = (counter >> 8);
					twiData[1] = counter;
				break;
			}
